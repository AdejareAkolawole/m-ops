...
type Tab = "overview" | "performance" | "incidents" | "admin" | "insights" | "debug" | "sla"
  ...
  return (
    <div className="p-6 space-y-5 max-w-3xl">
          ...
          <div className="rounded-xl border border-zinc-100 dark:border-zinc-800 divide-y divide-zinc-100 dark:divide-zinc-800">
            {incidents.map((inc, i) => (
              <div key={i} className="flex items-center justify-between px-4 py-3 gap-4">
                ...
                <span className="text-xs font-mono text-red-500 shrink-0">{inc.durationMs ? fmt(inc.durationMs) : "ongoing"}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="text-[11px] text-zinc-400">
        Based on {history.length} checks for <strong>{projectName}</strong>.
      </div>
    </div>
  )
}
