...
// All keys are prefixed with userId to isolate data between accounts on the same browser
let _uid = "anon"
export function initStore(userId: string) {
  _uid = userId || "anon"
}

function key(k: string) { return `${_uid}:${k}` }
...
export function savePlanCache(plan: string, historyDays: number) {
  ...
  localStorage.setItem(key(PLAN_CACHE_KEY), JSON.stringify({ plan, historyDays }))
}

function getHistoryDays(): number {
  if (typeof window === "undefined") return 7
  try {
    const raw = localStorage.getItem(key(PLAN_CACHE_KEY))
...
export function getVercelAccount(): VercelAccount | null {
  ...
  try { return JSON.parse(localStorage.getItem(key(VERCEL_ACCOUNT_KEY)) || "null") } catch { return null }
}

export function saveVercelAccount(account: VercelAccount) {
  localStorage.setItem(key(VERCEL_ACCOUNT_KEY), JSON.stringify(account))
}

export function clearVercelAccount() {
  localStorage.removeItem(key(VERCEL_ACCOUNT_KEY))
  localStorage.removeItem(key(VERCEL_PROJECTS_KEY))
}
...
export function getVercelProjects(): VercelSyncedProject[] {
  if (typeof window === "undefined") return []
  try { return JSON.parse(localStorage.getItem(key(VERCEL_PROJECTS_KEY)) || "[]") } catch { return [] }
}

export function saveVercelProjects(projects: VercelSyncedProject[]) {
  localStorage.setItem(key(VERCEL_PROJECTS_KEY), JSON.stringify(projects))
}
...
export function getManualProjects(): ManualProject[] {
  if (typeof window === "undefined") return []
  try { return JSON.parse(localStorage.getItem(key(MANUAL_KEY)) || "[]") } catch { return [] }
}
...
export function saveManualProject(project: ManualProject) {
  ...
  localStorage.setItem(key(MANUAL_KEY), JSON.stringify(projects))
}

export function deleteManualProject(id: string) {
  localStorage.setItem(key(MANUAL_KEY), JSON.stringify(getManualProjects().filter((p) => p.id !== id)))
  const checks = getAllChecks()
  delete checks[id]
  localStorage.setItem(key(CHECKS_KEY), JSON.stringify(checks))
}
...
export function getAllChecks(): Record<string, CheckResult[]> {
  if (typeof window === "undefined") return {}
  try { return JSON.parse(localStorage.getItem(key(CHECKS_KEY)) || "{}") } catch { return {} }
}
...
export function pushCheck(projectId: string, check: CheckResult) {
  ...
  localStorage.setItem(key(CHECKS_KEY), JSON.stringify(all))
}
...
export function getVercelSelectedIds(): string[] | null {
  if (typeof window === "undefined") return null
  const raw = localStorage.getItem(key(VERCEL_SELECTED_KEY))
  if (raw === null) return null
  try { return JSON.parse(raw) } catch { return [] }
}

export function saveVercelSelectedIds(ids: string[]) {
  localStorage.setItem(key(VERCEL_SELECTED_KEY), JSON.stringify(ids))
}

export function clearVercelSelectedIds() {
  localStorage.removeItem(key(VERCEL_SELECTED_KEY))
}
...
export function getProjectAdminConfig(projectId: string): ProjectAdminConfig {
  if (typeof window === "undefined") return {}
  try { return JSON.parse(localStorage.getItem(key(`hub_admin_cfg_${projectId}`)) || "{}") } catch { return {} }
}

export function saveProjectAdminConfig(projectId: string, cfg: ProjectAdminConfig) {
  localStorage.setItem(key(`hub_admin_cfg_${projectId}`), JSON.stringify(cfg))
}
...
export function getProviderAccount(provider: HostingProvider): ProviderAccount | null {
  if (typeof window === "undefined") return null
  try { return JSON.parse(localStorage.getItem(key(`hub_provider_${provider}`)) || "null") } catch { return null }
}

export function saveProviderAccount(account: ProviderAccount) {
  localStorage.setItem(key(`hub_provider_${account.provider}`), JSON.stringify(account))
}

export function clearProviderAccount(provider: HostingProvider) {
  localStorage.removeItem(key(`hub_provider_${provider}`))
  localStorage.removeItem(key(`hub_provider_projects_${provider}`))
}
...
export function getProviderProjects(provider: HostingProvider): ProviderProject[] {
  if (typeof window === "undefined") return []
  try { return JSON.parse(localStorage.getItem(key(`hub_provider_projects_${provider}`)) || "[]") } catch { return [] }
}

export function saveProviderProjects(provider: HostingProvider, projects: ProviderProject[]) {
  localStorage.setItem(key(`hub_provider_projects_${provider}`), JSON.stringify(projects))
}
...
export function getGitHubAccount(): GitHubAccount | null {
  if (typeof window === "undefined") return null
  try { return JSON.parse(localStorage.getItem(key("hub_github_account")) || "null") } catch { return null }
}
export function saveGitHubAccount(a: GitHubAccount) {
  localStorage.setItem(key("hub_github_account"), JSON.stringify(a))
}
export function clearGitHubAccount() {
  localStorage.removeItem(key("hub_github_account"))
}
...
export function getProjectRepo(projectId: string): GitHubRepo | null {
  if (typeof window === "undefined") return null
  try { return JSON.parse(localStorage.getItem(key(`hub_repo_${projectId}`)) || "null") } catch { return null }
}
export function saveProjectRepo(projectId: string, repo: GitHubRepo) {
  localStorage.setItem(key(`hub_repo_${projectId}`), JSON.stringify(repo))
}
...
export function getAnalysis(projectId: string): AnalysisResult | null {
  if (typeof window === "undefined") return null
  try { return JSON.parse(localStorage.getItem(key(`hub_analysis_${projectId}`)) || "null") } catch { return null }
}
export function saveAnalysis(result: AnalysisResult) {
  localStorage.setItem(key(`hub_analysis_${result.projectId}`), JSON.stringify(result))
}
